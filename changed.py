...
if 'source_lang' not in st.session_state:
    st.session_state.source_lang = "Tshangla"

# Language selection with swap button
col1, col2, col3 = st.columns([2, 1, 2])
with col1:
    # Create a radio button using the current language from session state
    languages = ["Tshangla", "English"]
    current_index = 0 if st.session_state.source_lang == "Tshangla" else 1
    selected_language = st.radio("Source language:", languages, index=current_index, horizontal=True)
    
    # Update session state when radio button changes
    if selected_language != st.session_state.source_lang:
        st.session_state.source_lang = selected_language

with col2:
    st.write("##")
    if st.button("⇄ Swap"):
        # Simply swap the language and force rerun
        if st.session_state.source_lang == "Tshangla":
...
with col3:
    target_lang = "English" if st.session_state.source_lang == "Tshangla" else "Tshangla"
...
with tab1:
    user_input = st.text_input(f"Enter {st.session_state.source_lang} text:")
...
with tab2:
    st.write("Voice input can be used by recording your voice with another app, then typing the text here.")
    st.info("Note: If you can't use the built-in speech recognition, you can record your voice using a phone or other recording device, then manually type what you said.")
    
    voice_input = st.text_input("Enter your spoken text here:", key="voice_text_input")
...
# Translation logic
if user_input and translate_button:
    source_col = st.session_state.source_lang
    matches = find_match(user_input, source_col, df)
    ...
    if matches and matches[0][1] > 60:  # 60% similarity threshold
        ...
        with result_col1:
            st.markdown(f'<div class="language-container"><div class="language-header">{st.session_state.source_lang}</div>', unsafe_allow_html=True)
            st.write(best_match)
            
            # Better audio file handling
            source_audio = get_audio_file_path(st.session_state.source_lang, match_id)
            if source_audio:
